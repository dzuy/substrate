...
function buildFallbackProgressSummary(entries: ProgressSummaryEntry[]) {
  ...
  if (scored.length === 0) {
    return 'Complete a few check-ins to start seeing a meaningful progress pattern.';
  }
  ...
  return `Your recent pattern is ${direction} across the saved check-ins. Keep watching which daily inputs line up with stronger or weaker days.`;
}
