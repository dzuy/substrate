...
export default function CatalogWorkspace() {
  ...
  function openProduct(product?: CatalogProduct) {
    guard(() => { const next = product ? toForm(product, recordMap.get(product.id)) : { ...emptyForm }; setForm(next); setBaseline(JSON.stringify(next)); setEditor(true); setTab('overview'); setIngredientQuery(''); setError(''); setNotice(''); });
  }
  ...
  const editorTabs: { id: EditorTab; name: string; disabled?: boolean }[] = [
    { id: 'overview', name: 'Overview' }, { id: 'ingredients', name: 'Ingredients' }, ...Object.entries(evidenceSections).map(([id, section]) => ({ id: id as EditorTab, name: section.label, disabled: !form.record })),
    ...
    <div className="cms-body" inert={editor || !!confirm}>
      <aside className="cms-sidebar" aria-label="Catalog navigation and filters">
        ...
        <nav className="cms-nav" aria-label="Admin workspace">
          ...
          <button className={cx('cms-nav-item', workspace === 'sources' && 'active')} onClick={() => navigate('sources')}><FileText size={17} /><span>Source library</span><small>{records.filter((r) => !r.product_id).length}</small></button>
        </nav>
    ...
    {editor ? <ProductDrawer suspended={!!confirm} onClose={() => guard(() => setEditor(false))}>
        ...
        {notice ? <div className="cms-banner success" role="status"><Check size={15} /><span>{notice}</span><button aria-label="Dismiss notification" onClick={() => setNotice('')}><X size={16} /></button></div> : null}
          ...
          <div className="cms-editor-scroll" role="tabpanel" aria-label={editorTabs.find((t) => t.id === tab)?.name}>
            ...
            {tab === 'ingredients' ? <Section title="Ingredient library" description="Maintain normalized ingredients separately from the original formula text."><div className="cms-inline-note"><FileText size={17} /><span>The original INCI is retained under Formula & sources. These linked ingredients support structured product data.</span></div><div className="cms-ingredient-search"><Field name="Find or create an ingredient"><input placeholder="Search the ingredient library…" value={ingredientQuery} onChange={(e) => setIngredientQuery(e.target.value)} /></Field><div className="cms-ingredient-options">{ingredients.filter((i) => !form.ingredients.some((f) => f.ingredientId === i.id) && i.name.toLowerCase().includes(ingredientQuery.toLowerCase())).slice(0, 8).map((i) => <button key={i.id} className="cms-button compact" onClick={() => attachIngredient(i.id)}><Plus size={12} />{i.name}</button>)}{ingredientQuery.trim() ? <button className="cms-button compact" onClick={() => void addIngredient()}>Create “{ingredientQuery.trim()}”</button> : null}</div></div>
